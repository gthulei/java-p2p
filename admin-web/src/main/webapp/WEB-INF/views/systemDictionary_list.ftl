...
<head>
	...
	$(function(){
      ...
      $(".del_Btn").click(function(){
        ...
        $.messager.confirm("提示","是否删除名称为"+v[0]+"的数据？",function(){
          $.ajax({
            type: "POST",
            url: "/systemDictionaryIdDel.json",
            dataType: "json",
            data: {
              id: v[1]
            },
					...
					<table class="table">
						...
						<#list pageResult.data as vo>
							<tr>
								...
								<td>${vo.sn}</td>
								<td>
									<a href="javascript:void(-1);" class="edit_Btn" data="${vo.id},${vo.title},${vo.sn}">修改</a>
                  <a href="javascript:void(-1);" class="del_Btn" style="color: red" del_data="${vo.title},${vo.id}">删除</a>
								</td>
							</tr>
						</#list>
						</tbody>
