			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/rechargeOffline">
					...
					<div class="form-group">
					    ...
					    <select class="form-control" name="bankId">
					    	...
					    	<#list banks as bank>
	    						<option value="${bank.id}">
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${bank.bankname}"]+")${bank.accountname}${bank.bankforkname}";
										document.write(str);
									</script>
								</option>
	    					</#list>
					    ...
					    <script type="text/javascript">
					    	$("[name=bankId] option[value='${(qo.bankId)!''}']").attr("selected","selected");
					    </script>
					</div>
					<div class="form-group">
					    <label>交易时间</label>
				...
				<div class="panel panel-default">
                  ...
                  var str="("+SITE_BANK_TYPE_NAME_MAP["${info.bankinfo.bankname!''}"]+")${info.bankinfo.accountname!''}${info.bankinfo.bankforkname!''}";
                  document.write(str);
                </script>
								</td>
								<td>${info.audit}</td>
